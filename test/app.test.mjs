...
test("static app includes every primary control and asset reference", async () => {
  ...
  assert.match(script, /const TRACK_WIDTH = 64/);
  ...
  assert.match(script, /name: "Lesmo 2"/);
  assert.match(script, /name: "Variante Ascari"/);
  assert.match(script, /name: "Curva Alboreto"/);
  ...
  assert.match(script, /const before = pointOnTrack\(projected\.progress - CHECKPOINT_TANGENT_SAMPLE\)/);
  assert.match(script, /const after = pointOnTrack\(projected\.progress \+ CHECKPOINT_TANGENT_SAMPLE\)/);
  assert.match(script, /const geometry = checkpointGeometry\(projected\.x, projected\.y, angle, HALF_TRACK \+ CHECKPOINT_OVERHANG\)/);
